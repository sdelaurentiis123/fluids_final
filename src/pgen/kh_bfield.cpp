  ...
  if (iprob == 2) {
    ...
    for (int k=ks; k<=ke; k++) {
      for (int j=js; j<=je; j++) {
        for (int i=is; i<=ie; i++) {
          Real current_rho;
          if (pcoord->x2v(j) >= 0.0) { // Top layer x2 >= 0
            current_rho = 1.0;
          } else { // Bottom layer x2 < 0
            current_rho = drat;
          }
          phydro->u(IDN,k,j,i) = current_rho;

          Real vx = vflow*std::tanh((pcoord->x2v(j))/a);
          Real vy = amp*std::cos(TWO_PI*pcoord->x1v(i))
                                 *std::exp(-(SQR(pcoord->x2v(j)))/SQR(sigma));

          phydro->u(IM1,k,j,i) = vx * current_rho;
          phydro->u(IM2,k,j,i) = vy * current_rho;
          phydro->u(IM3,k,j,i) = 0.0;
    ...
    if (MAGNETIC_FIELDS_ENABLED) {
      ...
      Real by0 = pin->GetOrAddReal("problem","by0", 0.0);
      ...
      // add initial magnetic energy to energy density (internal energy is from P=1)
      if (NON_BAROTROPIC_EOS) {
        for (int k=ks; k<=ke; k++) {
          for (int j=js; j<=je; j++) {
            for (int i=is; i<=ie; i++) {
